...
function generateRandomPerks(numSurvivors, includeKiller) {
    ...
    for (var i = 1; i <= numSurvivors; i++) {
        var survivorPerks_1 = [];
        for (var _i = 0, _a = Object.values(SurvivorPerkCategory); _i < _a.length; _i++) {
            var category = _a[_i];
            var perk = getRandomPerk(category);
    ...
    if (includeKiller) {
        var killerPerks_1 = [];
        for (var _b = 0, _c = Object.values(KillerPerkCategory); _b < _c.length; _b++) {
            var category = _c[_b];
            var perk = getRandomPerk(category);
